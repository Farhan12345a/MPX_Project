...
{  
  int i;
  //Kernel level interupt handlers
  ...
  // 60(Meaning interupt number) is generated when a process calls sys_rec(IDLE)
  idt_set_gate(60,(u32int)sys_call_isr, 0x08, 0x8e); 
  idt_set_gate(0x24,(u32int)serial_io_isr, 0x08, 0x8e); //summary document intruction (interupt section)
