...
void kmain(void)
{
 ...
 mpx_init(MODULE_R3);
   //PART TWO BELOW 
 mpx_init(MEM_MODULE); //Lets system know memory module to implemented
 //mpx_init(IO_MODULE); // enable R6
   sys_set_malloc(allocateMemory); //telling OS to use our memory alloc
   ...
   init_paging();
   klogv("Creating memory list"); 
   createmList(); //creates list 
   klogv("Initializing heap");
   initializeHeap(HEAP_SIZE);
   klogv("Memory initialization done");
...
\_| |_/\___|_| .__/ \____/\___/|_| |_| |_|
             |_|                          
*/
   char help1[] = " _   _      _       _____\n| | | |    | |     /  __ \\\n| |_| | ___| |_ __ | /  \\/ ___  _ __ ___\n|  _  |/ _ \\ | '_ \\| |    / _ \\| '_ ` _ \\\n| | | |  __/ | |_) | \\__/\\ (_) | | | | | |\n\\_| |_/\\___|_| .__/ \\____/\\___/|_| |_| |_|  OS\n             |_|\n";
   int helpone = 242;
   

   createAList();
   initQueues();
   com_open(&e_flag,9600);//opening serial port
   loadIdle();
   loadComhand();
   loadTimeDaemon();
   yield();
   sys_req(WRITE, DEFAULT_DEVICE, "\033[92m", &helpone);
   sys_req(WRITE, DEFAULT_DEVICE, help1, &helpone);
