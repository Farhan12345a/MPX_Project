...
sys_call_isr: ;;;All the pushes below store the context of the registers.
	pusha   ;;; store all general purpose registers
	push ds ;;; store the segment registers
	push es 
	push fs	;;;Syscall gets the registers in context.h from pushing these registers.
	push gs
	push esp ;;; storing the stack pointer register
	call sys_call ;;; calls sys_call which performs the context switching
	mov esp, eax ;;; move the return value from sys call into esp 
	pop gs	;;;popping segement registers
	pop fs 
	pop es
	pop ds
	popa ;;;popping general purpose register
	iret
