...
//if idle or exit generates interrupt 60
...
/*
  ...
  Params..: int cur_mod (symbolic constants MODULE_R1, MODULE_R2, 			etc.  These constants can be found inside
			mpx_supt.h
*/
// If return is true use memory module we implmented. Call is in kmain
...
*/
//if memory module implemented calls student malloc , if not just calls kmalloc
...
/*
  ...
  If memory module is active, call student made memory module(R5)
*/
 ...
 * --------------------------------
 * Calls sysreq to display 
 * a user entered message
 * without a starting a newline.
 * @param message character pointer pointing to text inside message.
